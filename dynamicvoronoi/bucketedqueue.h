...
public:
   ...
   */
    ...
    int getNumBuckets() { return buckets.size(); }  ///std::map
...
private:
    ...
    typedef std::map< int, std::queue<T> > BucketType;  //队列
    BucketType buckets;
    typename BucketType::iterator nextPop;  //类型转换放在类定义中？？
};
