...
void DynamicVoronoi::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap) {
    ...
    sizeX = _sizeX;
    sizeY = _sizeY;
    data = new dataCell*[sizeX];//创建sizeX大小的数组 数组元素为dataCell*
    for (int x=0; x<sizeX; x++)
    ...
    //创建初始化使用的dataCell
    dataCell c;
    c.dist = INFINITY;  //float型 为了程序安全，永远不要使用INFINITY(或(numeric_limits<float>::max)())做求相反数或比较大小或加减乘等运算。
    c.sqdist = INT_MAX;
...
void DynamicVoronoi::initializeMap(int _sizeX, int _sizeY, bool** _gridMap) {
    gridMap = _gridMap;
    initializeEmpty(_sizeX, _sizeY, false); //创建空的地图，(x,y)

    for (int x=0; x<sizeX; x++) {
        for (int y=0; y<sizeY; y++) {
            if (gridMap[x][y]) {            //如果gridmap_中的(x,y)被占用了
                dataCell c = data[x][y];    //如果c没有被占用，即data_中的(x,y)没被占用，需要更新
                if (!isOccupied(x,y,c)) {   //如果在gridmap_中的邻居元素全被占用
